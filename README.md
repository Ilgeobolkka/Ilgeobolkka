...
### 2026-08-04 SCRUM-426 기술 드라이런

세 시연 계정의 역할을 로컬 데스크톱 브라우저와 MySQL에서 검증했습니다. 아래는 SCRUM-426의
자동·브라우저·DB 기술 드라이런 결과이며, 후속 전체 실행 검증은
[SCRUM-424 실행 근거](./docs/evidence/release-verification/scrum-424-2026-08-04.md)에 분리해 보존합니다.

- Reader A: 미대여 페이지 열기 `201`, 콘텐츠 조회 `200`, 잔액 `96 → 95`, 차감 내역과 30일 대여
  각 1건 생성을 확인했습니다.
- Reader B: 0잉크 상태에서 `422 INSUFFICIENT_INK`, 충전 안내와 잔존 로딩 문구 부재를 확인했고,
  잔액·내역·대여·열람 세션·서재는 변하지 않았으며 콘텐츠는 제공되지 않았습니다.
- Reader C: 온라인 소장 도서 페이지 열기와 콘텐츠 조회가 성공했고, 잔액·내역·대여는 변하지 않았습니다.
- 세 경로의 브라우저 console warning/error는 0건이었습니다.

검증 시나리오와 출시 게이트는 [테스트 전략](./docs/test-strategy.md)을 따릅니다. 자격 증명과 세션
식별자는 저장소와 검증 기록에 남기지 않습니다.

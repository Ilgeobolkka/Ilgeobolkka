...
- 비즈니스 로직·예외 케이스·실현 가능성 관점 팀 검토(SCRUM-86)로 아래 지점을 AI 초안에서 수정했습니다.
  ...
  - **`ReadingSession` 동일 페이지 재진입 처리**(결정 절 엔티티 설명): 초안은 세션 교체 시 이전
    ...
    계약으로 구체화됩니다.
  - **`ConfirmedPage` 확정 순서·잠금 생략**(결정 절 엔티티 설명): 초안은 유니크 제약으로 중복 차감을
    막는다는 설명만 있고 처리 순서·잠금 여부가 없었습니다. 확정 처리는 이 행의 존재 여부를 먼저
    확인해 이미 확정된 페이지면 `PointAccount`를 잠그지 않고 차감 없이 응답하며, 동시 확정 경합으로
    유니크 제약을 위반하면 이미 다른 요청이 확정한 것으로 간주하도록 추가했습니다. — 판단 근거:
    INV-002(중복 차감 금지)는 유지하면서 이미 확정된 페이지의 재확인 요청(T-BILL-006·007)까지 행을
    잠그는 것은 불필요한 경합 비용이라 AGENTS.md 2절(단순함 우선)에 어긋납니다. 구체적 순서는
    ADR-0008 "확정 요청 처리 순서" 절에서 확정합니다.

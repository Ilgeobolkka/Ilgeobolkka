...
- **열람 세션 이력 관리 — 세션마다 새 행과 `ReadingSessionStatus`(ACTIVE/REPLACED)를 남기는 방식 vs
  ...
  요청 두 개가 거의 동시에 도착하는 경합은, 유니크 제약 위반 예외를 같은 트랜잭션에서 잡아 다시
  조회하는 방식 대신 `reader_id` 유니크 키에 대한 DB upsert(`INSERT ... ON DUPLICATE KEY UPDATE`)
  한 문장으로 처리합니다. 조회·삽입·예외 복구를 애플리케이션에서 분기하지 않고 단일 원자적 구문으로
  세션 행을 삽입하거나 갱신하므로, 참여 중인 트랜잭션이 rollback-only 상태가 되어 마지막 커밋이
  실패하는 위험이 없습니다. 동시 접속량이 낮은 MVP 규모에서 이 방식은 별도 분산 락 없이 실현
  가능합니다(conventions.md 주석과 과한 설계 제한 절).
...
- **`ReadingSession`**: 사용자당 하나의 온라인 열람 세션(VIEW-002)과 열람 확정 6초 판정의 기준
  ...
  요청을 자연스럽게 무효화합니다. 세션 오픈은 `reader_id` 유니크 키에 대한
  upsert(`INSERT ... ON DUPLICATE KEY UPDATE`)로 처리해, 동시 최초 오픈 경합에서도 유니크 제약
  예외를 잡지 않고 삽입·갱신을 원자적으로 수행합니다. 같은 세션 안에서 현재 페이지와 동일한 `pageNumber`로 페이지 이동
...
- **`ConfirmedPage`**: 열람 확정 페이지(BILL-002). `(reader_id, book_id, page_number)` 유니크
  ...
  페이지라면 `PointAccount`를 잠그지 않고 차감 없이 응답하고, 미확정이면 `PointAccount` 행을 잠근 뒤
  `ConfirmedPage`를 다시 조회(더블 체크)해 그 사이 다른 요청이 확정했으면 차감 없이 응답합니다.
  유니크 제약 위반 예외를 잡아 처리하지 않으며, 유니크 제약은 최종 DB 안전망으로만 둡니다
  (순서와 응답 값은 ADR-0008 참고).
...
- 비즈니스 로직·예외 케이스·실현 가능성 관점 팀 검토(SCRUM-86)로 아래 지점을 AI 초안에서 수정했습니다.
  ...
  - **`ReadingSession` 동시 오픈 처리**(고려한 대안 절, 결정 절 엔티티 설명): 초안은 "사용자당 한
    행을 유지하는 방식을 선택했습니다"로 끝나 같은 사용자의 세션 오픈 요청 두 개가 거의 동시에
    도착하는 경우를 다루지 않았습니다. 1차 검토(SCRUM-86)에서는 "삽입 경합으로 유니크 제약을
    위반하면 이미 삽입된 행을 다시 조회해 갱신"하는 방식을 추가했으나, 후속 리뷰에서 **유니크 제약
    예외를 같은 트랜잭션에서 잡아 다시 조회하는 방식은 참여 중인 트랜잭션이 rollback-only 상태가 되어
    마지막 커밋에서 실패할 수 있다**는 지적을 받아, `reader_id` 유니크 키에 대한 DB
    upsert(`INSERT ... ON DUPLICATE KEY UPDATE`) 한 문장으로 처리하도록 계약을 구체화했습니다. —
    판단 근거: 리뷰가 제시한 세 대안(행 잠금 / DB upsert / 별도 트랜잭션 재시도) 중, 세션이
    `reader_id`당 한 행인 이 모델에서는 upsert가 조회·삽입·예외 복구 분기를 없애 가장 단순하고
    트랜잭션 오염 위험이 없습니다. VIEW-002(사용자당 하나의 세션)를 동시 요청에서도 지키면서, MVP
    규모의 낮은 동시 접속량에서 별도 분산 락을 두지 않는 편이 conventions.md "주석과 과한 설계 제한"
    절 및 AGENTS.md 2절(단순함 우선)과 일치합니다.
  - **`ReadingSession` 동일 페이지 재진입 처리**(결정 절 엔티티 설명): 초안은 세션 교체 시 이전
    세션이 무효화된다는 설명만 있고, 같은 페이지로 다시 페이지 이동 요청이 오는 경우를 다루지
    않았습니다. 현재 페이지와 같은 `pageNumber` 요청은 페이지 이동으로 보지 않고 `page_opened_at`을
    갱신하지 않는다고 명시했습니다. — 판단 근거: ADR-0001의 6초 판정 기준을 지키려면 같은 페이지
    재진입마다 타이머가 초기화되면 안 되며(BILL-003과 같은 원칙), 이 처리는 ADR-0008에서 API
    계약으로 구체화됩니다.
  - **`ConfirmedPage` 확정 순서·잠금 조건**(결정 절 엔티티 설명): 초안은 유니크 제약으로 중복 차감을
    막는다는 설명만 있고 처리 순서·잠금 여부가 없었습니다. 확정 처리는 이 행의 존재 여부를 먼저
    확인해 이미 확정된 페이지면 `PointAccount`를 잠그지 않고 차감 없이 응답하고, 미확정이면
    `PointAccount`를 잠근 뒤 `ConfirmedPage`를 다시 조회(더블 체크)해 처리하도록 정리했습니다.
    (후속 리뷰 반영: 1차 검토에서 쓴 "유니크 제약 위반 시 이미 확정된 것으로 간주" 표현은 예외 캐치
    방식과 혼동되어, ADR-0008에서 확정한 "`PointAccount` 잠금 후 `ConfirmedPage` 재조회" 방식에
    맞춰 문장을 통일했습니다.) — 판단 근거: INV-002(중복 차감 금지)는 유지하면서 이미 확정된
    페이지의 재확인 요청(T-BILL-006·007)까지 행을 잠그는 것은 불필요한 경합 비용이라 AGENTS.md
    2절(단순함 우선)에 어긋납니다. 구체적 순서는 ADR-0008 "확정 요청 처리 순서" 절에서 확정합니다.
  - **`PointAccount` 잠금 조건**(결정 절 엔티티 설명): 초안은 "차감 시 이 행을 잠그고"라고만 했고
    잠금을 얻는 조건이 없었습니다. 이 잠금은 `ConfirmedPage`가 아직 없을 때만 획득하며, 이미 확정된
    페이지의 재확인 요청에서는 잠그지 않는다고 명시했습니다. — 판단 근거: 위 `ConfirmedPage` 항목과
    같은 근거로, INV-004(음수 잔액 금지)는 신규 차감 시에만 필요한 보호이므로 재확인 요청까지
    잠그면 과한 설계입니다(AGENTS.md 2절).

...
- 비즈니스 로직·예외 케이스·실현 가능성 관점 팀 검토(SCRUM-86)로 아래 지점을 AI 초안에서 수정했습니다.
  ...
  - **`ReadingSession` 동시 오픈 처리**(고려한 대안 절): 초안은 "사용자당 한 행을 유지하는 방식을
    선택했습니다"로 끝나 같은 사용자의 세션 오픈 요청 두 개가 거의 동시에 도착하는 경우를 다루지
    않았습니다. `reader_id`로 조회해 있으면 갱신·없으면 삽입하되, 삽입 경합으로 유니크 제약을
    위반하면 이미 삽입된 행을 다시 조회해 갱신하는 방식을 추가했습니다. — 판단 근거: VIEW-002
    (사용자당 하나의 세션)를 동시 요청에서도 지키면서, MVP 규모의 낮은 동시 접속량에서는 증거 없이
    별도 분산 락을 두지 않는 편이 conventions.md "주석과 과한 설계 제한" 절 및 AGENTS.md 2절(단순함
    우선)과 일치합니다.

...
- 비즈니스 로직·예외 케이스·실현 가능성 관점 팀 검토(SCRUM-86)로 아래 지점을 AI 초안에서 수정했습니다.
  ...
  - **확정 요청 처리 순서·재시도 응답값 확정**(새 "확정 요청 처리 순서" 절, INV-002 문단,
    ...
    구체적 처리 순서·잠금 시점이 없었습니다. 활성 세션 검증 → 6초 경과 검증 →
    `ConfirmedPage` 존재 확인(있으면 무잠금 응답) → `PointAccount` 잠금 후 차감 →
    유니크 제약 경합 시 재확인 응답의 5단계 순서를 신설했습니다. 이미 확정된 페이지 재확인 시
    `ConfirmReadingResponse.deductedAmount=0`, `balanceAfter`는 요청 시점 현재 잔액임을 명시하고,
    같은 페이지로 재진입하는 `PATCH` 요청은 `page_opened_at`을 유지해 6초 판정이 초기화되지 않게
    했습니다. — 판단 근거: INV-002(중복 차감 금지)·INV-003(원자성)·INV-004(음수 잔액 금지)를
    동시에 만족시키려면 `ConfirmedPage` 확인이 `PointAccount` 잠금보다 먼저 와야 하고(불필요한

...
로컬 개발과 테스트는 Docker Compose MySQL을 사용하지만 실제 배포는 Amazon RDS for MySQL을 사용합니다.
두 환경을 위해 애플리케이션 빌드를 나누거나 접속 정보를 코드에 넣으면 설정이 어긋나고 시크릿이 노출될 위험이 있습니다. 운영 RDS 연결에는 로컬과 다른 네트워크 주소와 TLS 검증이 필요합니다.

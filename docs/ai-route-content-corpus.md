...
- `ai-route-v2` manifest는 최종적으로 새 비소설 90권과 이전 버전 PDF·SHA-256을 그대로 쓰는 소설 10권을
  합친 100권을 기록합니다. 확장 중에는 그 부분 집합을 기록하며, 검증과 적재는 manifest에 실제로 든
  도서만 대상으로 합니다. 전체 페이지 수는 manifest의 도서별 페이지 수 합계로 계산합니다.
...
- 같은 개념을 표현만 바꿔 반복하지 않고, 중복 후보를 평가할 수 있는 제한된 유사 페이지는 의도와
  근거를 메타데이터에 표시합니다.
- 선수 관계를 얕게 겁니다. 경로에 페이지 하나를 넣으면 그 페이지의 전이적 선수가 모두 따라오므로,
  선수를 촘촘히 걸면 어떤 예산으로도 열 수 없는 페이지가 생깁니다. 두 기준을 함께 지킵니다.
  - 전이 폐쇄가 5페이지 이하인 후보 페이지가 서로 다른 장 3개 이상을 덮습니다. 가장 작은 예산과
    빠른 깊이(각각 5페이지)로 만들 경로의 재료입니다.
  - 후보 페이지의 75% 이상이 전이 폐쇄 15페이지 이하입니다. 가장 큰 예산과 깊은 깊이(각각 15페이지)로도
    닿지 못하는 페이지가 4분의 1을 넘지 않게 합니다.
- 후보 페이지와 전이적 선수 폐쇄를 합친 묶음에는 같은 중복 그룹 페이지가 둘 이상 없어야 합니다. 최종
  경로는 선수 폐쇄를 모두 포함하면서 중복 그룹당 최대 한 페이지만 포함해야 하므로, 이 조건을 어긴 후보는
  예산이나 조립 순서와 무관하게 실현할 수 없습니다.
...
각 지원 도서에는 대표 독서 목적 1개를 만들며, 평가 케이스 수는 manifest의 지원 도서 수와 같습니다
(100권 완성 시 90개). 이 데이터는 저장소에서 버전 관리하고 프롬프트·후보 정책 회귀와 출시 전 품질
확인에 함께 사용합니다. 독립적인 미관측 평가셋이 아니므로 결과를 일반 사용자 전체의 성과로 확대
해석하지 않습니다.
...
열어야 해서 어느 배분으로도 고를 수 없습니다. 셋째, 한 절을 무관과 무관 아님으로 가르지 않습니다. 절의
`primaryConcepts[]`는 하나이므로 같은 개념이 절 안에서 목적에 맞기도 하고 아니기도 할 수는 없고, 가르면
무관이 아닌 페이지가 무관 페이지를 선수로 밟아야만 닿습니다. 어느 쪽으로 온전하게 만들지는 본문 문단이
무엇을 주제로 삼는지로 정합니다. 어느 문단도 필수·도움 개념을 주제로 삼지 않으면 절째 무관으로 넓히고,
한 문단이라도 주제로 삼으면 그 절은 무관일 수 없으므로 먼저 넣은 페이지를 뺍니다.
...
- 대표 목적 전체를 `bookId ASC`로 실행하며 건수를 세지 않되 일곱 평가 시나리오가 모두 포함되어야
  합니다. 예산 0은 정답 데이터가 지정한 활성 대여 페이지를 입력하고, 나머지 비소장 시나리오는 활성
  대여가 없습니다.
...
- 선수 순서 위반률은 정답 데이터의 전체 필수 선수 관계를 분모로 사용합니다. 생성한 경로가 그 관계를
  얼마나 다뤘는지와 무관하게 분모를 고정하며, 경로가 짧다고 분모가 줄지 않습니다.
- 관계 `선수 -> 의존` 하나는 생성한 경로에 의존 페이지가 있을 때 판정합니다. 의존 페이지가 있는데 선수
  페이지가 경로에 없으면 `필수 선수 누락`, 선수 페이지가 의존 페이지보다 뒤에 있으면 `역순`으로 각각
  위반 한 건입니다. 의존 페이지가 경로에 없는 관계는 준수도 위반도 아니며 분모에만 남습니다.
  분모를 다룬 관계 수로 좁히면 페이지를 적게 고를수록 지표가 좋아지고, 반대로 다루지 않은 관계까지
  위반으로 세면 정답 경로를 그대로 재현하라는 요구가 되어 정확한 페이지 일치보다 개념 충족을
  우선한다는 기준과 어긋납니다.
...
8. 비웹 평가로 manifest에 든 대표 목적 전체를 실행하고 PRD의 품질 기준을 통과한 비소설 콘텐츠만
   AI 경로 지원으로 활성화합니다.

## 완료 조건

- 검증은 도서 권수를 세지 않고 manifest에 실제로 든 도서만 아래 계약대로 검사합니다. 100권 완성 여부는
  확장 진행 상황이지 적재 통과 조건이 아니며, 부분 집합도 계약을 지키면 적재할 수 있습니다.
- manifest의 지원 도서가 모두 48~72페이지이고 최소 6개 장을 가집니다.
...
- 모든 페이지에 선수 관계·중복 그룹 소속 목록 필드가 하나씩 있고, 선수 관계는 같은 도서·콘텐츠 버전의
  존재하는 페이지만 가리키며 자기 참조와 방향 순환이 없습니다. 모든 후보와 전이적 선수 폐쇄에는 같은
  중복 그룹 페이지가 둘 이상 없습니다.
...
- manifest의 대표 목적 전체가 [AI 잉크 경로 PRD의 품질·출시 기준](./prd/ai-ink-route.md#품질과-출시-기준)을

...
[ADR-0016](./adr/content/0016-publish-korean-original-ebook-content-for-ai-fixtures.md)을 따릅니다.
...
- 새로 제작하는 비소설 90권은 각각 원본 PDF 기준 48~72페이지입니다. 모든 도서를 같은 길이로 만들지
  않고 `contentVersion`, `bookId`, 기존 제목의 결정적 seed로 범위 안에서 길이를 다르게 배정합니다.
...
- 비소설은 기존 제목·저자·카테고리·소개에 맞춘 한국어 창작 전자책으로 편집합니다. 표지·판권·머리말·목차,
  최소 6개 장의 이어지는 본문, 사례·한계·맺음말·참고 자료를 포함합니다.
- 독자용 본문에는 제작 과정이나 페이지 역할을 설명하는 반복 문장을 넣지 않습니다. 화면 본문과 AI 분석
  텍스트는 분리하고 영어는 URL·원전 고유 서지정보처럼 추적에 필요한 곳으로 제한합니다.
- 권리 검증 공개 원전은 책별 참고 자료로만 추적하고 원문·번역문·스캔·삽화를 PDF 본문에 직접 싣지
  않습니다. 배치 보존 이미지 페이지는 새로 작성한 한국어 도표만 사용합니다.

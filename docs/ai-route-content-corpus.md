...
- 한 책 안에 핵심 개념, 선수 개념, 적용 사례, 반례와 결론 역할의 페이지를 포함합니다. 장 시작에는
...
- 핵심 개념과 평가 대상 내용은 도서별로 구분합니다. 도서별 구체 근거를 먼저 정하고, 카테고리 문체와
  장 전개를 그 근거에 맞춰 작성합니다. 평가용 중복 후보만 의도와 근거를 메타데이터에 표시합니다.
...
- 비소설은 기존 제목·저자·카테고리·소개에 맞춘 한국어 창작 전자책으로 편집합니다. 표지·판권·머리말·목차,
  최소 6개 장의 이어지는 본문, 구체 사례·반례·맺음말·관련 고전 읽을거리를 포함합니다.
...
- 권리 검증 공개 원전은 책별 **관련 고전 읽을거리**로만 추적합니다. 본문 사실의 출처나 실제 집필에
  사용한 자료라고 주장하지 않으며 원문·번역문·스캔·삽화를 PDF 본문에 직접 싣지 않습니다.
  배치 보존 이미지 페이지에는 새로 생성한 사진 또는 새로 작성한 한국어 도표·삽화 등 내용에 맞는 시각 자료를 사용합니다.
- 파일별 권리 상태와 개인정보 부재를 확인한 뒤 Git fixture로 반입합니다.

`source-ledger.json`은 관련 고전의 서지·권리 정보와 한국어 창작 본문의 관계를 분리합니다.
`usage.bodyFactSourceClaimed=false`로 본문 출처 주장이 없음을 명시하고, `editorialEvidence[]`에는 각 장에
실제로 들어간 도서별 구체 근거와 페이지 연결만 기록합니다. 관련 고전과 본문 사이에 직접 인용·번역 관계가
없는데도 이를 연구 근거나 사실 출처로 기록해서는 안 됩니다.
...
| `cases[]` 정답 | `requiredConcepts[]`, `helpfulConcepts[]`, `requiredPrerequisites[]`, `irrelevantPageNumbers[]`, `irrelevantRationales[]`, `duplicatePageGroups[]`, `referencePageNumbers[]`, `referenceRationales[]`, `allowedAlternativePageNumbers[]` |
...
대표 목적은 도서 제목이나 `requiredConcepts[]`를 축자 포함하지 않습니다. 대신 목적에 적은 서로 다른
구체 근거가 정답 페이지 본문과 각각 연결되어야 합니다. 각 도서의 `referencePageNumbers[]`는 서로 다른
비연속 위치를 사용하고, 각 정답과 무관 페이지는 `referenceRationales[]`와 `irrelevantRationales[]`에서
목적의 어느 단서와 어떤 페이지 근거로 판정했는지 설명합니다. 무관 페이지는 검색 제외 또는 목적 근거
불일치 중 하나를 기록하며, 검색 대상 무관 페이지의 근거가 목적에 그대로 포함되면 실패합니다.
`contentRole` 분포를 맞추기 위해 본문상 근거가 없는 페이지를 정답에 추가하지 않습니다.
...
1. 기존 메타데이터에서 책의 주제·독자 수준·도서별 구체 근거를 확정합니다.
2. 비소설 도서별 48~72페이지를 충족하는 장·절, 카테고리 문체, 개념 지도와 선수 관계를 설계합니다.
3. 도서별 구체 근거를 장면·수치·이름·조건으로 풀어 본문과 이미지 원고를 작성하고 PDF를 조립합니다.
4. 실제 본문을 읽고 페이지별 역할·공개 가이드 주제와 도서별 대표 목적·정답·판정 근거를 작성합니다.
...
- 실제 PDF 본문에서 개념어와 숫자를 치환한 문장 골격의 65% 이상, 소제목 골격의 50% 이상이 고유해야
  합니다. 같은 30자 이상 문장은 최대 2권에만 나타나고, 한 도서에서 다른 5권 이상과 공유하는 문장 골격은
  15% 이하여야 하며, 서로 다른 두 도서의 정규화한 장 테마 순서가 완전히 같아서는 안 됩니다.
...
- 대표 목적에 도서 제목·정답 개념의 축자 누출이 없고, 90개 기준 페이지 집합이 모두 다른 비연속 위치이며
  모든 정답·무관 페이지의 판정 근거가 목적의 구체 단서와 실제 페이지 내용에 연결됩니다.

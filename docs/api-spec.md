...
- 도서 목록·검색·상세와 잉크 잔액·내역 조회, 잉크 구매 준비·완료, 소장 결제 준비 Controller,
  PortOne JVM SDK 결제 조회·웹훅 서명 검증 및 멱등 완료 경계까지 구현됐습니다. 소장 결제 완료·내역과
  나머지 도메인 기능은 아직 구현 전입니다.
...
| `GET /signup` | 회원가입 | 아니오 |
| `GET /login` | 로그인 | 아니오 |
| `GET /books/{bookId}/viewer` | 현재 도서 뷰어 | 예 |

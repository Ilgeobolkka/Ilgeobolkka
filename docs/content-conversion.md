...
입력 `fixtures/content/manifest.json` 파일 바이트의 SHA-256을 배치 식별자로 사용합니다. 기본 출력은
`var/content/pages/<manifestSha256>/`이고 이 디렉터리의 `manifest.json`에는 입력 manifest SHA,
Poppler 버전, 이미지 형식·DPI·품질, 도서별 원본 SHA와 400개 페이지 결과를 기록합니다.

변환 중에는 같은 출력 루트의 숨김 staging 디렉터리만 사용합니다. 전체 검증이 끝나면 staging
디렉터리를 최종 배치 식별자 경로로 이동합니다. 같은 manifest 결과가 이미 있으면 결과 manifest와
100개 IMAGE 파일의 바이트가 모두 같은 경우에만 재사용하고, 하나라도 다르면 DB 적재 전에 실패합니다.

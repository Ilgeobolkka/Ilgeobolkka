...
현재 저장소의 PNG를 포함해 시드가 만드는 `TEXT`·`IMAGE` 페이지는 원본 PDF에서 변환한 제품 콘텐츠가
아니며, 이 문서의 페이지 일대일 연결·manifest·첫/중간/마지막 페이지 대조·이미지 품질을 통과했다는
근거로 사용할 수 없습니다.

[ADR-0006](./adr/content/0006-use-poppler-and-private-s3-for-image-pages.md)의 Git 제외 로컬 파일시스템과
비공개 S3 저장 계약은 실제 원본 기반 `IMAGE` 콘텐츠에 적용합니다. 합성 시연 fixture는 운영 데이터에서
참조하지 않으며 HTTP 정적 자산으로 직접 공개하지 않습니다.

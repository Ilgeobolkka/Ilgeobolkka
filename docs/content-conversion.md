...
- 허용 버전은 Poppler 26.05.0과 26.08.0입니다. 두 버전은 배치 이미지 100장의 SHA-256이 장마다 모두
  같고, 같은 표본의 `pdftotext` 텍스트도 페이지마다 SHA-256이 같습니다. 비교 결과는 아래
  [보존된 검증 근거](#보존된-검증-근거)에 있습니다.
- 목록에 버전을 더하려면 먼저 같은 표본을 그 버전으로 다시 변환해 `pdftoppm` 이미지와 `pdftotext`
  텍스트를 모두 비교하고 근거를 남깁니다. 근거가 없는 버전은 넣지 않습니다 — 26.07.0을 넣지 않은
  이유입니다.
- 한 배치는 한 툴체인으로 변환합니다. `pdftotext`와 `pdftoppm`의 버전이 서로 다르면 둘 다 허용
  목록에 있어도 거부합니다. 산출물 동일성은 같은 버전끼리만 확인했기 때문입니다.
- 한 버전으로 고정하지 않는 이유는
...
결과 manifest 비교에서 `pdftotextVersion`·`pdftoppmVersion`은 제외합니다. 허용 목록의 버전들은 같은
산출물을 내므로 기록한 버전 문자열이 다르다는 이유로 같은 배치를 다시 만들 필요가 없습니다. 대신 기존
배치에 기록된 버전이 허용 목록에 있는지 확인하고, 목록 밖이면 재사용하지 않고 실패합니다. 재사용한
배치의 `manifest.json`은 먼저 게시한 실행의 버전을 그대로 유지하므로, 재현할 때 읽는 값은 그 기록입니다.
...
| JPEG SHA-256 | 미보존 | `f6e0cc25eeffd62d2d7008d5ff2030e30fb94bd110e80e30e806d97371b21f2f` |
| PNG SHA-256 | 미보존 | `5d0188cb75df5547338187fba719c75f803fd09c8b08688730d369874d265315` |

표본의 26.05.0 산출물 파일은 보존하지 않아 그때의 측정값인 바이트 수·픽셀 크기만 남아 있습니다.
26.08.0 SHA-256은 위 명령을 그대로 다시 실행해 얻은 값이며(2026-08-13 측정), 로컬에서는 Homebrew가
주는 26.08.0만 조달되므로 26.05.0 쪽 표본 해시는 다시 만들 수 없습니다. 이미지의 **바이트 단위 동일성
근거는 아래 배치 100장 비교**입니다.

| 항목 | 26.05.0 배치 | 26.08.0 배치 |
| --- | --- | --- |
| 비교 대상 | `initial-v1` 이미지 100장 | 같은 배치를 26.08.0으로 재변환 |
| 동일 장수 | 100/100 | 100/100 |
| `book-001/page-002.jpg` SHA-256 | `6a2e5e41bbeb5d859d90d73eed21c08972769726378010c87f17046e9a75572e` | 같음 |
| 100장 SHA-256 목록의 SHA-256 | `8dfe99728d699e0646d6986ac69322d9f559b7df6dbf912d141adca165431d6d` | 같음 |

장별 SHA-256이 100장 모두 같았고, 상대 경로순 해시 목록을 다시 해시한 집계값도 양쪽이 같았습니다
(2026-08-13 측정). 실제 `initial-v1` 100권 400페이지 변환·적재도 26.08.0으로 통과했습니다.

텍스트 경로도 같은 표본으로 비교했습니다. 허용 목록은 `pdftoppm`뿐 아니라 `pdftotext`에도 적용되므로
[`book_001.pdf`](./evidence/page-image-quality/book_001.pdf) **18페이지 전부**를 변환기와 같은 명령
(`pdftotext -f <쪽> -l <쪽> -enc UTF-8 -eol unix <파일> -`)으로 추출해 페이지별 SHA-256을 비교했고
**18/18 페이지가 모두 동일**했습니다(2026-08-13 측정).

- 26.05.0: conda-forge 리눅스 빌드 — CI가 설치하는 경로
- 26.08.0: Homebrew macOS 빌드 — 개발 로컬 경로
- 3페이지 텍스트 SHA-256: `e8f5ad284b2c12c58400bcd1fc77ea75c767050fe1ffec8bf0c20cafd42cf614` (양쪽 동일)

즉 실제로 갈리는 조합(CI 리눅스 26.05.0 ↔ 로컬 macOS 26.08.0)에서 텍스트 산출물이 같습니다.

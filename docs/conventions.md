...
- 포인트 차감, 내역 생성, 열람 확정, 도서별 누적 사용 포인트, 소장 상태, 마지막 열람 위치 갱신은
...
- 잔액 부족, 중복 차감 금지, 도서 가격 상한, 소장 전환, 열람 시간 같은 도메인 규칙은 Service와 Entity에서 검증합니다.
...
- 페이지 기본 단가 50P, 도서 가격까지의 마지막 잔여액 차감, 소장 전환과 열람 확정 6초는 단순 운영 설정이 아니라 제품·도메인 계약입니다. 의미 있는 이름으로 표현하고 값을 바꿀 때 PRD, 테스트 전략, 관련 ADR을 함께 검토합니다.
...
- 도메인 규칙은 빠른 단위 테스트로, JPA·트랜잭션·동시성·고유 제약과 누적 사용 포인트 상한은 MySQL 통합 테스트로 검증합니다.

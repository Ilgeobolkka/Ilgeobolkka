...
- 포인트 차감, 내역 생성, 열람 확정 행 생성, `confirmedPageCount` 증가와 마지막 열람 위치 갱신은
...
- 잔액 부족, 중복 차감 금지, 열람 확정 페이지 수, 90% 소장 판정, 열람 시간 같은 도메인 규칙은 Service와 Entity에서 검증합니다.
...
- 페이지 고정 단가 50P, `ceil(totalPageCount × 0.9)` 소장 기준과 열람 확정 6초는 단순 운영 설정이
  아니라 제품·도메인 계약입니다. 의미 있는 이름으로 표현하고 값을 바꿀 때 PRD, 테스트 전략,
  관련 ADR을 함께 검토합니다.
...
- 이메일은 앞뒤 공백을 제거하고 `Locale.ROOT` 기준 소문자로 정규화한 값만 저장·조회합니다.
...
- 비밀번호는 최소 8자·UTF-8 최대 64바이트이며 영문, 숫자, 공백이 아닌 ASCII 특수문자를 각각
  한 개 이상 포함해야 합니다.
- 로그인 성공 시 세션 ID를 교체하고 2시간 비활성 시 만료합니다. 로그아웃 시 로그인 세션과 도메인의
  `ReadingSession`을 함께 무효화합니다.
- Spring Security의 CSRF 보호를 끄지 않습니다. Thymeleaf 폼은 hidden field, JavaScript `fetch`는
  페이지의 meta 태그에서 읽은 토큰을 요청 헤더로 전달합니다.
- 로그인 실패는 이메일 존재 여부와 관계없이 `INVALID_CREDENTIALS`로 통일합니다.

## 목록과 이미지 전달

- 도서 목록·검색은 `title ASC, id ASC`, 포인트 내역은 `occurredAt DESC, id DESC`로 정렬해 같은 값의
  순서까지 고정합니다. 두 목록 모두 한 페이지에 10개를 제공합니다.
- 검색어는 앞뒤 공백을 제거하고 `%`, `_`를 SQL 와일드카드가 아닌 일반 문자로 이스케이프합니다.
- 브라우저에 S3 주소를 노출하지 않습니다. same-origin 이미지 Controller가 인증된 독자의 현재
  `ReadingSession`과 현재 페이지를 검증한 뒤 이미지 바이트를 전달합니다.
- 페이지 이미지 응답은 개인 열람 자산이므로 공유 캐시에 저장하지 않게 `Cache-Control: private, no-store`를 사용합니다.
...
- 도메인 규칙은 빠른 단위 테스트로, JPA·트랜잭션·동시성·고유 제약, 확정 페이지 수 일치와 90% 소장
  경계는 MySQL 통합 테스트로 검증합니다.

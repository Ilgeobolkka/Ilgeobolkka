...
`pdftotext`와 `pdftoppm`은 [허용 버전](./content-conversion.md#이미지-변환-기준)인 Poppler
`26.05.0` 또는 `26.08.0`이어야 하며, 두 명령의 버전이 서로 같아야 합니다. 먼저 실제 실행 경로와 버전을
확인합니다.
...
콘텐츠 manifest SHA-256, 콘텐츠 manifest·평가 데이터 Git 리비전, 임베딩 모델·경로 모델·후보 정책·prompt·schema 버전과 지표를 평가 결과에 기록합니다.

`ai-route-v2`는 평가를 통과한 뒤 같은 `contentVersion`·데이터 정책 프로필의 manifest 지원 도서 전체를 한
트랜잭션으로 활성화하고 소설 10권은 `false`로 둡니다. AI 경로 지원 활성화가 없는 초기 manifest 배치는 이
단계를 건너뜁니다. 활성화까지 끝나면 시연 계정 비밀번호를 `.env`에
...
이 절은 로컬뿐 아니라 공개 환경 배포에도 적용합니다. 재적재 분기가 위 [콘텐츠 변환·적재](#콘텐츠-변환적재)
절차를 그대로 재사용하므로 이어서 둡니다.

[재현 정보 일곱 항목](./prd/ai-ink-route.md#재평가와-지원-활성화-순서) 중 하나가 바뀌면 대표 목적 전체를
전체 재평가합니다. 판정과 금지 사항은
[재평가와 지원 활성화 순서](./prd/ai-ink-route.md#재평가와-지원-활성화-순서)가 정본이며, 아래는 환경
실행 순서입니다. 순서는 재평가에 콘텐츠 재적재가 포함되는지로만 갈리고, 여러 항목이 함께 바뀌면 재적재를
포함하는 쪽을 따릅니다.

분기와 무관하게, 임베딩 모델이든 경로 모델이든 **모델을 하나라도 바꿨으면**
[OpenAI 데이터 정책 프로필](./evidence/openai-data-policy/README.md)을 먼저 재확인하고, 조건이
달라졌으면 사용자 안내와 콘텐츠 지원 조건을 갱신한 뒤에 진행합니다. 적재 절차의 프로필 일치 검사는
동일성 확인이지 공식 출처 재확인을 대신하지 않습니다.

manifest 리비전이나 임베딩 모델이 바뀌어 **콘텐츠 재적재가 포함되는** 재평가는 공개 전에만 실행합니다.
사용자 기록과 HTTP 트래픽이 없는 새 후보 DB를 준비하고, 위 최초 적재와 같은 순서(콘텐츠 배치 → 대표
목적 전체 평가 → 통과 시 지원 도서 활성화 → 일반 서버 시작)를 그대로 따릅니다. 이미 HTTP 트래픽을 받아 사용자
기록이 생긴 환경의 콘텐츠 버전을 바꾸는 것은 위 롤백 금지와 같은 이유로 2차 MVP 범위가 아닙니다.
따라서 최초 공개 이후에 manifest 리비전이나 임베딩 모델을 바꾸는 경로는 2차 MVP에 없습니다.
`dataPolicyVersion`도 manifest의 결정적 입력이므로 공개 후 프로필 상향은 이 경로에 걸립니다. 이때는
`OPENAI_DATA_POLICY_VERSION`만 먼저 올리지 말고([DB와 값이 달라 일반 서버가 기동에 실패합니다](#openai-데이터비용-제어))
[`AI_ROUTE_ENABLED=false`](#openai-데이터비용-제어)로 AI 경로를 끈 뒤 별도 결정으로 처리합니다.

재적재를 포함하지 않는 다섯 항목(평가 데이터 리비전, 경로 모델, 후보 정책·prompt·schema 버전) 중 하나
이상만 바뀐 **콘텐츠 재적재가 없는** 재평가는 다음 순서로 실행합니다.

1. 위 모델 재확인이 필요한 경우라면 먼저 끝냅니다.
2. 후보 정책·prompt·schema는 새 버전 식별자를 정본에 올리고 기존 식별자를 재사용하지 않습니다. 경로
   모델은 모델 이름 자체가 식별자이므로 별도 발급이 없습니다.
3. 바뀐 코드·설정을 올린 **HTTP 트래픽을 받지 않는 애플리케이션 실행 환경**에서 대표 목적 전체를 재평가합니다.
   여기서 환경은 실행 인스턴스를 뜻하며 DB가 아닙니다. 평가는 콘텐츠 읽기 전용이므로 기존 콘텐츠 DB를
   그대로 사용하고, [별도 평가 DB·평가 사용자·결제 기록](./prd/ai-ink-route.md#사용자와-지원-도서)을
   새로 만들거나 공개 DB를 복제하지 않습니다. 권한·예산 시나리오는 평가 입력으로 직접 제공합니다.
4. 전체 기준을 통과하면 코드와 설정만 배포합니다. `contentVersion`과 `ai_route_supported`는 바꾸지
   않으며 활성화를 실행하지 않습니다.

바뀐 항목이 평가 데이터 리비전뿐이면 4번에서 배포할 산출물이 없으므로, 3번과 같은 조건의 실행 환경에서
대표 목적 전체를 다시 실행해 결과만 갱신합니다.

미통과 시 배포하지 않습니다. 후보 DB는 HTTP 트래픽을 받기 전이므로 폐기할 수 있습니다.

...
`ai-route-v2`는 평가를 통과한 뒤 같은 `contentVersion`·데이터 정책 프로필의 비소설 90권을 한
...
[재현 정보 일곱 항목](./prd/ai-ink-route.md#재평가와-지원-활성화-순서) 중 하나가 바뀌면 대표 목적 90개를
...
사용자 기록과 HTTP 트래픽이 없는 새 후보 DB를 준비하고, 위 최초 적재와 같은 순서(콘텐츠 배치 → 90개
평가 → 통과 시 비소설 90권 활성화 → 일반 서버 시작)를 그대로 따릅니다. 이미 HTTP 트래픽을 받아 사용자
...
3. 바뀐 코드·설정을 올린 **HTTP 트래픽을 받지 않는 애플리케이션 실행 환경**에서 90개를 재평가합니다.

...
`V1` migration은 아래 잉크·30일 대여·도서 원가 직접 결제 모델의 11개 테이블과 핵심 제약을 생성합니다.
`V2`~`V6` migration은 [AI 잉크 경로 2차 MVP 구현 모델](#ai-잉크-경로-2차-mvp-구현-모델)의 도서·페이지
확장, 생성·저장 경로·현재 경로·일일 사용량 테이블과 후속 제약을 추가합니다. 두 모델은 JPA Entity와 실제
외래 키 관계로 매핑됐고 관련 API까지 구현됐습니다. 기존 migration은 수정하지 않고 후속 migration으로만
스키마를 변경합니다.
...
## AI 잉크 경로 2차 MVP 구현 모델

이 절은 [AI 잉크 경로 PRD](./prd/ai-ink-route.md)와
[AI 구현 계약](./api-spec.md#ai-잉크-경로-2차-mvp-구현-계약)을 구현한 현재 논리·물리 모델입니다. 아래
컬럼과 테이블은 `V2`~`V6` migration과 JPA Entity에 반영됐습니다.
...
| `no_route_reason` | `VARCHAR(40) CHARACTER SET ascii COLLATE ascii_bin` | 예 | - | `NO_RELEVANT_PAGES`, `INSUFFICIENT_BUDGET`, `INSUFFICIENT_DEPTH` 중 하나 |
...
`NO_RELEVANT_PAGES`와 소장 깊이 부족 `INSUFFICIENT_DEPTH`이면 `minimum_required_ink`가 `NULL`,
`INSUFFICIENT_BUDGET`이면 선택 예산보다 큰 최소 추가 잉크를 가집니다. 다른 상태에서는 두 필드가 모두
`NULL`입니다.

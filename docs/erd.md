...
생성하고, 11개 JPA Entity와 실제 외래 키 기반 최소 연관관계까지 매핑했습니다. 잉크 계좌·원장 도메인과
저장 경계, 잉크 구매 준비·완료 API, 소장 결제 준비·`PENDING` 재사용, 소장 결제 완료와 `BookOwnership`
생성, 완료된 소장 결제 내역 조회, PortOne 결제 조회·웹훅 멱등 완료 경계까지 구현됐으며 페이지 대여와
나머지 조회 API는 아직 구현 전입니다.

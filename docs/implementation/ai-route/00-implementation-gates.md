...
- 재평가는 재현 정보 일곱 항목 중 하나가 바뀌었을 때 대표 목적 90개 전체를 다시 실행하는 것입니다. 일부
...
- 재적재가 포함되면 새 후보 DB에 `false`로 적재하고 통과 뒤 비소설 90권을 한 transaction으로 활성화한
...
- 금지: 재평가를 이유로 한 공개 버전의 자동 비활성화·재활성화, 미통과 조합의 공개 환경 배포, 90권 중
  일부만 활성화, 다른 version 조합의 기존 report 재사용, 데이터 정책 프로필 재확인 없는 모델 배포,

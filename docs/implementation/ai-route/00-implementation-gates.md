...
- 금지: 평가 정답의 runtime 입력 사용, v2 값의 무버전 변경, 재시도 후보·정책 교체, 임시 prompt·schema와
  중복 version 타입
...
- 거부 응답과 생성 결과 재조회에는 재계산한 비용·권한 상세를 노출하지 않습니다(재조회는 생성 시점
  스냅샷 그대로). 저장 경로 상세 조회의 재계산은 기존 계약 그대로입니다. 생성 시점 스냅샷은
  [ADR 0016](../../adr/domain/0016-store-generation-item-cost-status.md)에 따라
  `ai_route_generation_item`에 저장합니다.

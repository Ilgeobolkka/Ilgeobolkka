...
- [.env.example](../../../../.env.example)에 네 환경변수 이름과 빈 기본값이 있고,
  [application.yaml](../../../../src/main/resources/application.yaml)이 이를 설정 Properties에 바인딩합니다.
- `OpenAiConfiguration`은 실행 모드에 따라 조건부 `openAiRestClient` Bean을 제공하고,
  초기 `initial-v1` content-import에서는 실제 외부 요청 직전까지 OpenAI 설정 검증을 미룹니다.
- Embeddings·Responses adapter는 아직 구현되지 않았습니다.
...
3. 초기 `initial-v1` content-import는 OpenAI 값 없이 유지하고 OpenAI Gateway를 호출하지 않습니다.
   ...
   F03이 제공하는 `openAiRestClient`가 외부 요청 직전에 이를 호출합니다. `contentVersion`은 콘텐츠 적재
   작업이 명시적으로 분기합니다.
...
F04·F05 담당자에게 Bean 이름과 검증된 Properties API를 전달합니다. 후속 adapter는 `openAiRestClient`를
주입받아 사용하고 `RestClient.builder()` 등으로 별도 client를 만들어 공통 설정과 검증을 우회하지 않습니다.

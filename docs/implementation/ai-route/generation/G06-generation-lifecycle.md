...
- 만료 필터는 시작 경로의 **세 조회 모두**에 있습니다. 처음에는 한도 초과 직전 재조회에만 빼 두고
  "만료 행이 올 길이 없다"는 논증을 주석에 적었는데, 리뷰에서 깨졌습니다. 이 메서드에 닿기 전에
  사용량 행 잠금을 기다리고 그 대기는 앞 transaction이 쥔 만큼 길어지므로, 기다리는 사이 같은 키가
  시작·완료되고 보관 기간까지 지날 수 있습니다. 잠금 조회는 read view가 아니라 최신 commit을 읽어서
  그 만료 행을 그대로 집어 옵니다. **셋 중 하나라도 필터를 빼지 마세요.**
- 정리·복구 배치는 **고른 수와 처리한 수를 나눠 돌려줍니다**(`BatchOutcome`). 호출자는 반복 여부를
  고른 수로, 로그를 처리한 수로 판단합니다. 복구는 잠그기 전에 남이 정상 완료한 건을 건너뛰므로 두
  수가 갈리는데, 처리한 수로 반복을 판단하면 200건 중 한 건만 건너뛰어도 199가 돌아와 남은 backlog가
  통째로 다음 주기로 밀립니다. 밀리는 것은 이미 보관 기간을 넘겼을 수 있는 `GENERATING`입니다.
- **허용 파일 밖 변경** 셋입니다.
  1. `AiRouteGenerationStartService`와 그 통합 테스트(G05 소유) — 완료 조건의 만료 판정 요구 때문.
     승인 완료. 리뷰 반영으로 한도 초과 경로의 만료 필터와 그 경합 테스트를 같은 근거로 한 번 더
     고쳤습니다.

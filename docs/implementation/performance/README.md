...
- 상태: Runbook 6단계 완료·정확성/품질 미달·기준선 충족으로 단순 구성 유지

실제 작업은 [순차 실행 runbook](./RUNBOOK.md)에 따라 한 단계씩 진행한다. 각 단계의 종료 조건을 AI가
검증해 보고하고, 사람의 확인 없이 다음 단계로 넘어가지 않는다.

4단계의 조건부 기술 진입 판정과 미실행 근거는
[4단계 결과](../../evidence/performance/mvp1-stage4-2026-08-13/result.md)에 기록했다.
5단계의 깨끗한 최종 재측정과 비교 결과는
[5단계 결과](../../evidence/performance/mvp1-final-2026-08-15/result.md)에 기록했다. 첫 Spike 기능 실패
1건은 이후 같은 계획의 재실행과 후속 진단에서 재현되지 않아 잔여 위험으로 남기고 5단계를 조건부 통과로
닫았다. 다만 원인을 확인하지 못했으므로 최종 정확성·품질 게이트는 미달로 분리한다. 전체 부하를 다시
실행하지 않은 6단계 evidence·diff·민감정보·원시 artifact 감사와 로컬 인계는
[6단계 감사](../../evidence/performance/mvp1-final-2026-08-15/audit.md)에 기록했다.
...
| Warm-up | 5 RPS | 3분, 결과 제외 | JIT·풀·캐시 안정화 |
| Average | 20 RPS | 10분 × 3회 | 지속 기준선 |
| Peak | 50 RPS | 10분 × 3회 | 단기 피크 |
| Stress | 10→25→50→100→200 RPS | 단계당 3분, 1회 | 성능 변곡점과 병목 자원 확인 |
| Spike | 20→100→20 RPS | 2분→2분→5분, 1회 | 급증 뒤 회복 확인 |
| Soak | 20 RPS | 최종 30분, 1회 | 메모리·연결·스레드 누수 확인 |

Average·Peak의 각 실행 전에 Warm-up을 다시 수행하면 기준선의 고정 부하 시간은 1시간 33분이고, 최종의
고정 부하 시간은 여기에 Spike와 Soak를 더한 2시간 12분이다. 환경 복원·Smoke·동시성·브라우저·진단과
전체 검증을 포함해 기준선 2시간 30분, 최종 3시간 30분의 컴퓨터 전용 사용 시간을 확보한다.
...
| 개선 후보 | 대상 | 예상 효과 | 채택 기준 |
| --- | --- | --- | --- |
| 최신 이력 복합 인덱스 | 대여·원장·서재 성장 데이터 | 대상 SQL 50~95%, HTTP p95 10~60% 단축 | 제품 SQL 실행 계획 개선, 대상 endpoint 반복 측정의 일관된 p50·p95 개선, 정확성·쓰기 비회귀 |
| 신규 대여 쿼리 왕복 단축 | 페이지 첫 열기 | p95 20~45%, 처리량 20~60% 개선 | 실제 쿼리 왕복 감소, 대상 endpoint 반복 측정의 일관된 p50·p95 개선, 정확성·동시성 비회귀 |
...
| 정적 자산 압축·캐시 | 공개 화면 | 전송 bytes 20~70%, LCP 10~30% 개선 | 반복 측정의 일관된 전송 bytes 또는 Web Vital 개선, 보호 콘텐츠 cache 계약 유지 |

고정 15% 개선율은 어떤 후보에도 공통 채택 기준으로 사용하지 않는다. 절대 지연이 이미 낮은 경로에서는
유효한 실행 계획·왕복·전송량 개선을 가릴 수 있기 때문이다. 대신 후보가 직접 바꾸는 제품 지표를 같은
조건에서 3회 이상 반복하고, 모든 회차가 개선 방향이며 중앙값 개선이 측정 흔들림과 구분되는지 확인한다.
오류·dropped iteration·도메인 불변식·보안 계약·쓰기 흐름 회귀가 없고 추가 복잡도가 효과에 비해
낮을 때만 채택한다. 다중 인스턴스·가상 스레드처럼 별도 구조 비용 때문에 명시한 후보별 기준은 유지한다.
...
이미 정확성·잠정 품질 게이트를 만족하고, 근거가 있는 후보가 채택 기준을 충족하지 못했다면 수치를 만들기
위해 복잡도를 추가하지 않고 단순 구성을 유지하는 것도 완료로 판정한다.
...
- [ ] 효과가 채택 기준을 충족하지 못하거나 정확성이 깨지면 제거
...
8. 성능이 개선돼도 복잡도와 장애 지점이 늘면 채택 기준을 충족하지 않는 한 단순한 구성을 유지한다.

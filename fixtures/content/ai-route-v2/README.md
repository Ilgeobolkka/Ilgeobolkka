...
경제(`bookId` 41~50), 예술(`bookId` 61~70), 과학(`bookId` 21~30), 자기계발(`bookId` 91~100)은
카테고리 단위로 완성한 10권이다.
...
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **43건**, 일곱 시나리오를 모두 사용 (QUICK·BALANCED·예산 5·예산 10이 각 7건, DEEP이 6건, 예산 0·예산 15가 각 5건) |
...
비소설 43권 합계는 2,210페이지이며 그중 목차 43페이지, 이미지 231페이지, 본문 TEXT 1,936페이지다.
...
본문은 데스크톱 웹 뷰어 기준으로 `TEXT` 페이지당 272~701자, 43권 합계 약 83.3만 자다. 도서별 평균이

...
## 현재 상태: 51권 — 비소설 41 + 소설 10 (확장 중)

정본은 최종적으로 100권을 요구하지만 지금은 51권이 들어 있고, **이 상태로 적재하는 것이 현재
목표다.** 평가 시나리오 일곱 가지를 모두 채웠고, 남은 비소설 49권은 같은 절차로 늘린다. 소설 10권은
...
| 항목 | 정본 최종 목표 | 현재 |
| --- | --- | --- |
| 도서 수 | 100권 (비소설 90 + 소설 10) | **51권** (비소설 41 + 소설 10) |
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **41건**, 일곱 시나리오를 모두 사용 (BALANCED·예산 10 각 7건, QUICK·DEEP 각 6건, 예산 0·예산 5·예산 15 각 5건) |
| 소설 10권 | 기존 PDF·SHA-256 유지, `aiRouteCandidate=false` | **완료** (`bookId` 1~10) |

확장 시 비소설 49권을 `books[]`에, 평가 49건을 `cases[]`에 추가한다. 소설 10권은
...
비소설 41권 합계는 2,112페이지이며 그중 목차 41페이지, 이미지 237페이지, 본문 TEXT 1,834페이지다.
소설 10권 40페이지를 더하면 manifest 전체는 2,152페이지다.
...
본문은 데스크톱 웹 뷰어 기준으로 `TEXT` 페이지당 272~701자, 41권 합계 약 80.9만 자다. 도서별 평균이

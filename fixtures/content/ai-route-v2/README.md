...
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **35건**, 일곱 시나리오를 모두 사용 (QUICK·BALANCED가 각 6건, DEEP·예산 0·예산 10이 각 5건, 예산 5·예산 15가 각 4건) |
...
비소설 35권 합계는 1,809페이지이며 그중 목차 35페이지, 이미지 197페이지, 본문 TEXT 1,577페이지다.
소설 10권 40페이지를 더하면 manifest 전체는 1,849페이지다.
...
본문은 데스크톱 웹 뷰어 기준으로 `TEXT` 페이지당 272~701자, 35권 합계 약 70.2만 자다. 도서별 평균이
...
47개 중 11개가 60초다. 남은 55권은 **페이지당 345자 이상**을 기준으로 삼아 이 쏠림을 없앤다.

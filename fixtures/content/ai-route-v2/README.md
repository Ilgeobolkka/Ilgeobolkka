...
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **38건**, 일곱 시나리오를 모두 사용 (QUICK·BALANCED·예산 10이 각 6건, DEEP·예산 0·예산 5·예산 15가 각 5건) |
...
비소설 38권 합계는 1,960페이지이며 그중 목차 38페이지, 이미지 211페이지, 본문 TEXT 1,711페이지다.
소설 10권 40페이지를 더하면 manifest 전체는 2,000페이지다.
...
본문은 데스크톱 웹 뷰어 기준으로 `TEXT` 페이지당 272~701자, 38권 합계 약 74.8만 자다. 도서별 평균이

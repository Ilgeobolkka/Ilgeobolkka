...
경제(`bookId` 41~50), 예술(`bookId` 61~70), 과학(`bookId` 21~30)은 카테고리 단위로 완성한 10권이다.
여행(`bookId` 81~90)은 81번을 시작으로 만드는 중이다.
...
47개 중 11개가 60초다. 남은 57권은 **페이지당 345자 이상**을 기준으로 삼아 이 쏠림을 없앤다.

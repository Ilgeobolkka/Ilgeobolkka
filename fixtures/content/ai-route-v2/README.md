...
## 현재 상태: 53권 — 비소설 43 + 소설 10 (확장 중)

정본은 최종적으로 100권을 요구하지만 지금은 53권이 들어 있고, **이 상태로 적재하는 것이 현재
목표다.** 평가 시나리오 일곱 가지를 모두 채웠고, 남은 비소설 47권은 같은 절차로 늘린다. 소설 10권은
`initial-v1`의 PDF·SHA-256·페이지 수를 그대로 쓰며 이미 들어 있다.

경제(`bookId` 41~50), 예술(`bookId` 61~70), 과학(`bookId` 21~30), 철학(`bookId` 51~60),
여행(`bookId` 81~90)은 카테고리 단위로 완성한 10권이다.
...
| 항목 | 정본 최종 목표 | 현재 |
| --- | --- | --- |
| 도서 수 | 100권 (비소설 90 + 소설 10) | **53권** (비소설 43 + 소설 10) |
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **43건**, 일곱 시나리오를 모두 사용 (BALANCED 8건, QUICK·예산 10 각 7건, DEEP 6건, 예산 0·예산 5·예산 15 각 5건) |
| 소설 10권 | 기존 PDF·SHA-256 유지, `aiRouteCandidate=false` | **완료** (`bookId` 1~10) |

확장 시 비소설 47권을 `books[]`에, 평가 47건을 `cases[]`에 추가한다. 소설 10권은
...
비소설 43권 합계는 2,210페이지이며 그중 목차 43페이지, 이미지 249페이지, 본문 TEXT 1,918페이지다.
소설 10권 40페이지를 더하면 manifest 전체는 2,250페이지다.
...
본문은 데스크톱 웹 뷰어 기준으로 `TEXT` 페이지당 272~701자, 43권 합계 약 84.2만 자다. 도서별 평균이
...
47개 중 11개가 60초다. 남은 47권은 **페이지당 345자 이상**을 기준으로 삼아 이 쏠림을 없앤다.

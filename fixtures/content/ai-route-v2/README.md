...
## 현재 상태: 43권 — 비소설 33 + 소설 10 (확장 중)

정본은 최종적으로 100권을 요구하지만 지금은 43권이 들어 있고, **이 상태로 적재하는 것이 현재
목표다.** 평가 시나리오 일곱 가지를 모두 채웠고, 남은 비소설 57권은 같은 절차로 늘린다. 소설 10권은
...
| 항목 | 정본 최종 목표 | 현재 |
| --- | --- | --- |
| 도서 수 | 100권 (비소설 90 + 소설 10) | **43권** (비소설 33 + 소설 10) |
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **33건**, 일곱 시나리오를 모두 사용 (BALANCED가 6건, QUICK·DEEP·예산 10이 각 5건, 나머지 셋이 각 4건) |
| 소설 10권 | 기존 PDF·SHA-256 유지, `aiRouteCandidate=false` | **완료** (`bookId` 1~10) |

확장 시 비소설 57권을 `books[]`에, 평가 57건을 `cases[]`에 추가한다. 소설 10권은
...
비소설 33권 합계는 1,709페이지이며 그중 목차 33페이지, 이미지 189페이지, 본문 TEXT 1,487페이지다.
소설 10권 40페이지를 더하면 manifest 전체는 1,749페이지다.
...
본문은 데스크톱 웹 뷰어 기준으로 `TEXT` 페이지당 272~701자, 33권 합계 약 66.2만 자다. 도서별 평균이
...
| 425~590자 | 62 |
| 415~550자 | 21 · 27 · 63 · 69 · 70 |
| 370~555자 | 65 |
| 420~635자 | 68 |
| 370~570자 | 44 |
| 350~460자 | 26 |
| 345~440자 | 23 · 25 · 28 · 29 · 30 |
| 350~415자 | 24 |

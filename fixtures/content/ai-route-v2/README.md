...
경제(`bookId` 41~50), 예술(`bookId` 61~70), 과학(`bookId` 21~30), 철학(`bookId` 51~60)은
카테고리 단위로 완성한 10권이다.
...
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **43건**, 일곱 시나리오를 모두 사용 (QUICK·BALANCED·예산 10이 각 7건, DEEP·예산 5가 각 6건, 예산 0·예산 15가 각 5건) |
...
비소설 43권 합계는 2,201페이지이며 그중 목차 43페이지, 이미지 229페이지, 본문 TEXT 1,929페이지다.
소설 10권 40페이지를 더하면 manifest 전체는 2,241페이지다.

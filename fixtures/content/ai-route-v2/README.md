...
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **34건**, 일곱 시나리오를 모두 사용 (BALANCED·QUICK이 각 6건, DEEP·예산 10이 각 5건, 나머지 셋이 각 4건) |
...
비소설 34권 합계는 1,759페이지이며 그중 목차 34페이지, 이미지 193페이지, 본문 TEXT 1,532페이지다.
소설 10권 40페이지를 더하면 manifest 전체는 1,799페이지다.
...
본문은 데스크톱 웹 뷰어 기준으로 `TEXT` 페이지당 272~701자, 34권 합계 약 68.3만 자다. 도서별 평균이

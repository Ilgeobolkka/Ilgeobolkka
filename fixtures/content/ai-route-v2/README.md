...
| 평가 케이스 | 90건, 일곱 시나리오 전부 | **41건**, 일곱 시나리오를 모두 사용 (BALANCED·예산 10이 각 7건, QUICK·DEEP이 각 6건, 예산 0·예산 5·예산 15가 각 5건) |
...
비소설 41권 합계는 2,103페이지이며 그중 목차 41페이지, 이미지 221페이지, 본문 TEXT 1,841페이지다.
소설 10권 40페이지를 더하면 manifest 전체는 2,143페이지다.
...
본문은 데스크톱 웹 뷰어 기준으로 `TEXT` 페이지당 272~701자, 41권 합계 약 80.1만 자다. 도서별 평균이
...
47개 중 11개가 60초다. 남은 49권은 **페이지당 345자 이상**을 기준으로 삼아 이 쏠림을 없앤다.

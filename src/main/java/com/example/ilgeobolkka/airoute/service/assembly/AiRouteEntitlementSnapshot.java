 ...
 * 활성 대여 페이지 식별자를 함께 보존한다.
 */
public record AiRouteEntitlementSnapshot(
        boolean owned, int inkBalance, Set<Long> activeRentalPageIds) {

    public AiRouteEntitlementSnapshot {
        if (inkBalance < 0) {
            throw new IllegalArgumentException("잉크 잔액은 0 이상이어야 합니다.");
        }
        if (activeRentalPageIds == null) {
            throw new IllegalArgumentException("활성 대여 페이지 목록은 null일 수 없습니다.");
        }
        if (owned && (inkBalance != 0 || !activeRentalPageIds.isEmpty())) {
            throw new IllegalArgumentException("소장 권한 사본에는 잉크 잔액이나 활성 대여를 넣지 않습니다.");
        }
        if (activeRentalPageIds.stream().anyMatch(pageId -> pageId == null || pageId <= 0)) {
            throw new IllegalArgumentException("활성 대여 페이지 식별자는 양수여야 합니다.");
        }
        activeRentalPageIds = Set.copyOf(activeRentalPageIds);
    }
    ...
    public static AiRouteEntitlementSnapshot forNonOwned(
            int inkBalance, Set<Long> activeRentalPageIds) {
        return new AiRouteEntitlementSnapshot(false, inkBalance, activeRentalPageIds);
    }
}

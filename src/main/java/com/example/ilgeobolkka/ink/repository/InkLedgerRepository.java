...
public interface InkLedgerRepository extends Repository<InkLedger, Long> {
    ...
    Optional<InkLedger> findByInkPurchaseId(@Param("inkPurchaseId") long inkPurchaseId);

    @Query("SELECT ledger FROM InkLedger ledger WHERE ledger.pageRentalId = :pageRentalId")
    Optional<InkLedger> findByPageRentalId(@Param("pageRentalId") long pageRentalId);

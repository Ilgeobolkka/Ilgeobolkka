...
class AiRouteFeedbackApiMySqlIntegrationTest {
    ...
    private final AiRouteFeedbackFacade feedbackFacade;
    private final Clock clock;
    private static final Instant STARTED_AT = Instant.parse("2026-08-13T00:00:00.123456Z");

    @Autowired
    AiRouteFeedbackApiMySqlIntegrationTest(MockMvc mockMvc, JdbcTemplate jdbcTemplate, AiRouteFeedbackFacade feedbackFacade, Clock clock) {
    ...
    void 완료_owner의_세_rating_생성과_변경과_response_UTC_시각() throws Exception {
        ...
        mockMvc.perform(put("/api/ai-routes/{routeId}/feedback", routeId)
                ...
                .andExpect(jsonPath("$.feedbackAt").value("2026-08-13T00:00:00.123456Z"));
        ...
        mockMvc.perform(put("/api/ai-routes/{routeId}/feedback", routeId)
                ...
                .andExpect(jsonPath("$.feedbackAt").value("2026-08-13T00:01:00.123456Z"));
        ...
        mockMvc.perform(put("/api/ai-routes/{routeId}/feedback", routeId)
                ...
                .andExpect(jsonPath("$.feedbackAt").value("2026-08-13T00:02:00.123456Z"));
    ...
    @Test
    void 서로_다른_경로를_동시에_피드백해도_안전하다() throws Exception {
        long route1 = ROUTE_ID_BASE + 1;
        long route2 = ROUTE_ID_BASE + 2;
        완료한_경로를_생성한다(READER_ID, route1, "재무제표 읽기 1");
        완료한_경로를_생성한다(READER_ID, route2, "재무제표 읽기 2");

        AiRouteFeedbackRequest req1 = new AiRouteFeedbackRequest(AiReadingRouteFeedback.HELPFUL);
        AiRouteFeedbackRequest req2 = new AiRouteFeedbackRequest(AiReadingRouteFeedback.NEUTRAL);

        동시에_실행한다(
                () -> feedbackFacade.changeFeedback(route1, READER_ID, req1),
                () -> feedbackFacade.changeFeedback(route2, READER_ID, req2),
                List.of());

        Map<String, Object> state1 = jdbcTemplate.queryForMap(
                "SELECT feedback FROM ai_reading_route WHERE id = ?", route1);
        assertThat(state1.get("feedback")).isEqualTo("HELPFUL");

        Map<String, Object> state2 = jdbcTemplate.queryForMap(
                "SELECT feedback FROM ai_reading_route WHERE id = ?", route2);
        assertThat(state2.get("feedback")).isEqualTo("NEUTRAL");
    }
